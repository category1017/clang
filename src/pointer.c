...
void main(){
	...
	int cnt=0;//문자열 역순구하기에 사용되는 반복제한 횟수
	
	printf("문자열을 입력하세요: ");
	scanf("%s",p); //주소인덱스를 가지고 p변수를 사용하기 때문에 &가 없음. 
	len=strlen(p);
	printf("문자열의 길이는 %d 개 입니다.", len);
	...
	while(cnt<len){
		...
		putchar(*p);//현재 p인덱스 주소의 *p변수값을 1개씩 출력함수 puchar(1글자출력)
		printf("\n");
		cnt=cnt +1;//cnt++
	}
	putchar('\n');//글자를 출력하지 않고, 엔터치는 효과 /newline
	printf("출력한 이후의 인덱스 번호는 %d\n",p);//글자를 출력하지 않고, 엔터치는 효과 /newline
